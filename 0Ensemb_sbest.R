...
getacc<-function(svp,data){
  ...
  tabres<-table(result,data$Tey)
  tacc=(tabres[1,1]+tabres[2,2])/sum(tabres)
  return(tacc)
}
...
for (rep in 1:nrep){
  ...
  crossperf=data.frame(matrix(0, nrow=Ncv, ncol=6), row.names = NULL)
  colnames(crossperf) = c("Acc", "Spec","Sens","Fscore","Auc","TrainTime")
  ...
  for (crv in 1:Ncv)
  {
    ...
    btindx=which.max(bagginglistacc$Acc)
    baggingPredi[,length(bagginglist)+1]=baggingPredi[,btindx]
    ...
    crossperf$Acc[crv]=Acc
    crossperf$Spec[crv]=Spec
    crossperf$Sens[crv]=Sens
    crossperf$Fscore[crv]=Fscore
    crossperf$Auc[crv]=auc
    crossperf$TrainTime[crv]=ptm[3]
  }
  Repperf$Acc[rep]=mean(crossperf$Acc)
  Repperf$Spec[rep]=mean(crossperf$Spec)
  Repperf$Sens[rep]=mean(crossperf$Sens)
  Repperf$Fscore[rep]=mean(crossperf$Fscore)
  Repperf$Auc[rep]=mean(crossperf$Auc)
  Repperf$TrainTime[rep]=mean(crossperf$TrainTime)
}
Repperfsbest<-Repperf

#ROC curve
pred<-prediction(as.numeric(result),w2[,nfac+1])
ROCsbest <- performance(pred,measure="tpr", x.measure="fpr")
