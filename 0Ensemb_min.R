...
for (rep in 1:nrep){
  ...
  crossperfmin=data.frame(matrix(0, nrow=Ncv, ncol=6), row.names = NULL)
  colnames(crossperfmin) = c("Acc", "Spec","Sens","Fscore","Auc","TrainTime")
  ...
  for (crv in 1:Ncv)
  {
    ...
    for (i in 1:nrow(baggingPredi)){
      res=min(baggingPredi[i,1:length(bagginglist)])
    ...
    crossperfmin$Acc[crv]=Acc
    crossperfmin$Spec[crv]=Spec
    crossperfmin$Sens[crv]=Sens
    crossperfmin$Fscore[crv]=Fscore
    crossperfmin$Auc[crv]=auc
    crossperfmin$TrainTime[crv]=ptm[3]
  }
  Repperf$Acc[rep]=mean(crossperfmin$Acc)
  Repperf$Spec[rep]=mean(crossperfmin$Spec)
  Repperf$Sens[rep]=mean(crossperfmin$Sens)
  Repperf$Fscore[rep]=mean(crossperfmin$Fscore)
  Repperf$Auc[rep]=mean(crossperfmin$Auc)
  Repperf$TrainTime[rep]=mean(crossperfmin$TrainTime)
}
Repperfmin<-Repperf

#ROC curve
pred<-prediction(as.numeric(result),w2[,nfac+1])
ROCmin <- performance(pred,measure="tpr", x.measure="fpr")
